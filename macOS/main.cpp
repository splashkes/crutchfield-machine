...
static const char* UI_PATTERN_NAMES[] = {
    "H-bars", "V-bars", "dot", "checker", "gradient",
};
static constexpr int UI_PATTERN_COUNT =
    (int)(sizeof(UI_PATTERN_NAMES) / sizeof(UI_PATTERN_NAMES[0]));
...
static std::vector<UiControl> build_ui_controls() {
    ...
    ui_add_integer(controls, "noiseQ", "Noise Type", 0.0f, 3.0f, 1.0f,
        []() { return (float)S.noiseQ; }, [](float v) { S.noiseQ = (int)fmaxf(0.0f, fminf(3.0f, roundf(v))); },
        [](float v) { static const char* n[] = {"white", "pink", "grain", "scanline"}; return n[(int)fmaxf(0.0f, fminf(3.0f, roundf(v)))]; });
    ...
    ui_add_integer(controls, "pattern", "Pattern", 0.0f, (float)(UI_PATTERN_COUNT - 1), 1.0f,
        []() { return (float)S.p.pattern; },
        [](float v) { S.p.pattern = (int)fmaxf(0.0f, fminf((float)(UI_PATTERN_COUNT - 1), roundf(v))); sync_ddj_filter_leds(); },
        [](float v) { return UI_PATTERN_NAMES[(int)fmaxf(0.0f, fminf((float)(UI_PATTERN_COUNT - 1), roundf(v)))]; });
...
// ── keyboard ──────────────────────────────────────────────────────────────
static GLuint progFeedback = 0, progBlit = 0, hiresProg = 0;
...
static void key_cb(GLFWwindow*, int key, int scancode, int action, int mods) {
    ...
    if (S.ui.visible() && S.ui.key(key, action, mods)) return;
...
int main(int argc, char** argv) {
    ...
    hiresProg = build_blit_hires_program();
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (S.hiresPending) {
            ...
            save_screenshot_hires(latest.tex, hiresProg, S.simW, S.simH,
                                  g_captureScale, 1.0f);
            glViewport(0, 0, S.winW, S.winH);
        }
    ...
    if (hiresProg) glDeleteProgram(hiresProg);
