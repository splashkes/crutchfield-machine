...
struct Cfg {
    ...
    std::string midiPort;            // override the MIDI port hint (substring match)
    std::string midiUsage;           // path to a coalesced usage log (one row per action change)
};
...
static void print_cli_help() {
    printf(
      ...
      "  --midi-usage FILE   write a coalesced usage log to FILE — one line per action\n"
      "                      change, '---- CLEAR ----' dividers when ACT_CLEAR fires.\n"
...
// a single row, not 128. ACT_CLEAR fires emit a "---- CLEAR ----" divider
// so performance is visually separable from button-exploration.
...
static void apply_action(ActionId id, float mag) {
    ...
    switch (id) {
        ...
        // press, 0.0 on release). B is sticky (DISCRETE fires once per
        // press, each press flips the toggle).
        case ACT_MPK_HOLD:
