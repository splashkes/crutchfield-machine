...
enum ActionId : int {
    ...
    // Software-banked modes for controllers without enough physical
    // buttons (the MPK Mini Bank A/B toggle doesn't emit MIDI). Bound by
    // default to keyboard keys (Right Shift / Right Alt) but routable
    // from any source via bindings.ini.
...
public:
    ...
    struct MidiState {
        ...
        // has no spare hardware buttons. currentMpkMode() returns 0/1/2
        // derived from these (HOLD overrides B).
